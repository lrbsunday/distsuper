...
def start(force=False):
    if not force and os.path.exists(CONFIG.DISTSUPERAGENT.pid_file_path):
        logging.warning("当前目录下存在pidfile，请确保agent没有启动，"
                        "并删除pidfile后再试")
        sys.exit(-1)
    args = 'uwsgi --ini {}:distsuperagent'.format(CONFIG.config_file_path).split()
    env = os.environ
    env.update({'DISTSUPER_MODULE_NAME': 'agent'})
    ret = subprocess.Popen(args, env=env).wait()
    if ret == 0 and remote_check("127.0.0.1", CONFIG.DISTSUPERAGENT.port,
                                 'STARTING'):
...
def stop(force=False):
    ...
    if ret == 0 and not remote_check("127.0.0.1", CONFIG.DISTSUPERAGENT.port,
                                     'STOPPING'):
...
def restart(force=False):
    if not force and not os.path.exists(CONFIG.DISTSUPERAGENT.pid_file_path):
        logging.warning("找不到pidfile，请确保agent已启动，"
                        "并在pidfile所在路径下执行该命令")
        sys.exit(-1)
    args = 'uwsgi --reload {}'.format(CONFIG.DISTSUPERAGENT.pid_file_path).split()
    ret = subprocess.Popen(args).wait()
    if ret == 0 and remote_check("127.0.0.1", CONFIG.DISTSUPERAGENT.port,
                                 'STARTING'):
